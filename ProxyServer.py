    ...
    def get(self):
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response:
                ...
                main_proxy_server.error('ОШИБКА - Внутренняя ошибка прокси-сервера')
                self.set_status(500)
                self.write('Internal server error:\n' + str(e))
                self.finish()
...
class Configuration:
    ...
    def setconfig(self, port, mode=None, jscript=None):
        self.port = port
        self.mode = mode
    ...
    def __call__(self, port, mode=None, jscript=None, start_ioloop=True):
        ...
        try:
            if mode == 'jsinj':
                ...
                main_proxy_server.info("PROXY_SERVER запущен в режиме JSINJECTION")
            ...
            # Здесь не реализовано
                ...
                main_proxy_server.info("PROXY_SERVER запущен в режиме СБОР_СТАТИСТИКИ")
                app = tornado.web.Application([
                    (r'.*', ProxyHandler),
                ])
        except Exception as e:
            main_proxy_server.error('Неверные параметры скрипта при запуске PROXY_SERVER')
            sys.exit(1)
...
class WorkerProxyServer:
    ...
    def start(self):
        ...
        process_proxy.start()
        main_proxy_server.info('PROXY_SERVER запустился с PID = {0}'.format(process_proxy.pid))
    ...
    def stop(self):
        ...
        main_proxy_server.info('PROXY_SERVER остановлен')
